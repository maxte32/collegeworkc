...
static void Reverse(queueADT queue);
...
main()
{
    ...
    while (TRUE) {
        ...
        if (StringEqual(cmd, "enqueue")) {
            ...
            DisplayQueue(queue);
		} else if (StringEqual(cmd, "reverse")) {
			Reverse(queue);
...
static void GiveInstructions(void)
{
	...
	printf("  reverse     -- Reverse the contents of the queue\n");
...
static void DisplayQueue(queueADT queue)
{
    ...
    if (len == 0) {
        ...
        for (i = 0; i < len; i++) {
            if (i > 0) printf(", ");
            printf("%s", (char*)GetQueueElement(queue, i));
        }
    }
    printf("\n");
}
