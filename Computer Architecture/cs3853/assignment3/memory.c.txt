...
if (!directmapped && !fullyassociative){
	...
	for (i = 0; i < lines; i++){
		...
		if (setempty){
			...
			/* grabbing what is in the set before we replace */			
			for (i2 = 0; i2 < associativity; i2++){
				if (sets[setindexes[i]].blocks[i2].valid == 1){
			...
			if (hitflag == 0){
				...
				//}else if (setfull){
					...
					if (fifolru){
						...
						sets[setindexes[i]].blocks[accessindex].firstbrought = accesses;
		...
		if (tracing){					
			...
			if (setempty){
				...
				for (i2 = 0; i2 < arraytagsindex; i2++){
					if ( ( i2 == 0 ) && ( arraytagsindex == 1 )){
						printf (" %x(%d) ", arraytags[i2][0], arraytags[i2][1] );					
					} else if ( (i2 == 0) && ( arraytagsindex > 1 ) ){
						printf (" %x(%d), ", arraytags[i2][0], arraytags[i2][1] );					
					
					}else if ( i2 == (arraytagsindex -1)){
						printf (" %x(%d) ", arraytags[i2][0], arraytags[i2][1] );					
...
int tagmatch = 0;
int fullyarray[totalblocks][2];
int fullyarrayindex = 0;
for (i = 0; i < totalblocks; i++){
	fullyarray[i][0] = 0;
	fullyarray[i][1] = 0;
}
...
if (fullyassociative){
	...
	for (i = 0; i < lines; i++){
		...
		/* before you do anything copy what is in the cache right now */
		for (i2 = 0; i2 < totalblocks; i2++){
			if ( fully.blocks[i2].valid == 1  ){

				if (fifolru){
					fullyarray[fullyarrayindex][0] = fully.blocks[i2].tag;
					fullyarray[fullyarrayindex][1] = fully.blocks[i2].timebrought;
				} else {
					fullyarray[fullyarrayindex][0] = fully.blocks[i2].tag;
					fullyarray[fullyarrayindex][1] = fully.blocks[i2].firstbrought;
				}
		...
		if (tracing){
			...
			/* sort */
			for (c = 1 ; c <= fullyarrayindex - 1; c++) {
   			 	d = c;
 
    				while ( d > 0 && fullyarray[d][0] < fullyarray[d-1][0]) {
      					t          = fullyarray[d][0];
					t2	   = fullyarray[d][1];
      					fullyarray[d][0]   = fullyarray[d-1][0];
      					fullyarray[d][1]   = fullyarray[d-1][1];
      					fullyarray[d-1][0] = t;
       					fullyarray[d-1][1] = t2;
      					d--;
    				}
			...
			for (i2 = 0; i2 < fullyarrayindex; i2++){


				if ((i2 == 0) && (fullyarrayindex > 1)){

					printf (" %x(%d),", fullyarray[i2][0], fullyarray[i2][1] );					
				} else if ( (i2 == 0) && (fullyarrayindex == 1) ){ 
					printf (" %x(%d)", fullyarray[i2][0], fullyarray[i2][1] );					
				}else if ( i2 == ( fullyarrayindex -1 ) ){
					printf (" %x(%d)", fullyarray[i2][0], fullyarray[i2][1] );					
				} else {
					printf (" %x(%d),", fullyarray[i2][0], fullyarray[i2][1] );					
				}
