...
int main(int argc, char *argv[])
{
	...
	if(argc != 2){
		printf("Usage: udpserver [0.0-1.0]\n");
		return 0;
	}
...
//	 nbytessentb = sendto (sock, message, nbytes, 0,(struct sockaddr *) & name, size);
/*
	size = sendto (sock, buffer, nbytes, 0,(struct sockaddr *)&from, sizeof(from));
           if (size < 0)
             {
