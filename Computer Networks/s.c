...
int main(int argc, char **argv)
{
	...
	if(argc != 2){
		fprintf(stderr, "Usage: udpserver [0.0-1.0]\n");
		return -1;
	}
    ...
    */
	...
	if(prob >= ((double)rand() / (double)RAND_MAX)) // compute probability
        sendto(sock, buffer, nbytes, flags, (struct sockaddr *)&from,addrlen); // echo back if true
      }
#ifdef WIN
      system("PAUSE");
#endif
